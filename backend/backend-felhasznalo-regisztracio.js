...
function felhasznaloregisztracio(regisztraciosnev, regisztraciosjelszo, regisztraciosszerepkor, regisztraciosevfolyam, regisztraciososztalyjel){
...
con.connect(function (err) {
  ...
  con.query(`SELECT nev FROM felhasznalok WHERE nev = "${regisztraciosnev}";`, function (err, result) {
    ...
    {
      con.end();
      callback("Hiba a felhasználók lekérdezése közben!"+err);
    } 
    if (result[0].nev == regisztraciosnev)
    {
        con.end();
        callback("Hiba! Már létezik ilyen felhasználó!");
    }
    else
    {
        con.query(`INSERT INTO felhasznalok (nev, jelszo, szerepkor, evfolyam, osztalyjel) VALUES ("${regisztraciosnev}", "${regisztraciosjelszo}", "${regisztraciosszerepkor}", "${regisztraciosevfolyam}", "${regisztraciososztalyjel}")`, function (err, result) {
            ...
            {
              con.end();
              callback(`${regisztraciosnev}-t nem sikerült regisztrálni! Hiba: `+err);
            }
            con.end();
            callback(null, "Sikeres regisztráció!")
          });
    }
